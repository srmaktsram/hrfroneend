...
export class ClientsProfileComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.pipe(map(id => {
      ...
      this.clientId = id.id;
    }), mergeMap(() => this.http.get("http://localhost:8443/admin//clients/getDataClient" + "/" + this.adminId))).subscribe((data: any) => {
      this.allClients = data;

      this.clientData = this.allClients.filter(client => client.id == this.clientId);
  ...
  projects() {
    this.http.get("http://localhost:8443/admin/projects/getAdminproject" + "/" + this.adminId).subscribe((res: any) => {
      ...
      this.project = this.allProjects.filter(name => name.clientId == this.clientId)
